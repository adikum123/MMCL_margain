...
class NTXent(nn.Module):
    ...
    def forward(self, z, get_map=False):
        ...
        acc = accuracy(
            pred, torch.LongTensor(labels.reshape(n, m - 1)).to(logprob.device), m - 1
        )
...
class MMCL_INV(nn.Module):
    def __init__(
        ...
        self.eye = torch.eye(anchor_count * batch_size).cpu()
        ...
        self.oneone = (torch.ones(bs, bs) + torch.eye(bs) * reg).cpu()
        self.one_bs = torch.ones(batch_size, nn, 1).cpu()
        self.one = torch.ones(
            nn,
        ).cpu()
        self.KMASK = self.get_kmask(bs)
        self.block = torch.zeros(bs, 2 * bs).bool().cpu()
        self.block[:bs, :bs] = True
        self.block12 = torch.zeros(bs, 2 * bs).bool().cpu()
        self.block12[:bs, bs:] = True
        self.no_diag = (1 - torch.eye(bs)).bool().cpu()
        self.bs = bs
    ...
    def get_kmask(self, bs):
        KMASK = torch.ones(bs, bs, bs).bool().cpu()
        for t in range(bs):
            KMASK[t, t, :] = False
            KMASK[t, :, t] = False
        return KMASK.detach()
    ...
    def get_mask(self, batch_size, anchor_count):
        mask = torch.eye(batch_size, dtype=torch.float32).cpu()

        mask = mask.repeat(anchor_count, anchor_count)
        logits_mask = torch.scatter(
            torch.ones_like(mask),
            1,
            torch.arange(batch_size * anchor_count).view(-1, 1).cpu(),
            0,
        )
